DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_get_all_transfers_by_date_range`(
    IN inFromDate DATE,
    IN inToDate DATE
)
BEGIN
    ...
    IF (SELECT DATE(inFromDate))
    THEN
        SET inFromDate = inFromDate - INTERVAL 1 DAY;

        IF (SELECT DATE(inToDate))
        THEN
            SET inToDate = inToDate + INTERVAL 1 DAY;
            SET existDate = true;
        END IF;
    END IF;
    ...
    THEN
        ...
        WHERE
            DATE(trans.created_at) > DATE(inFromDate)
        AND DATE(trans.created_at) < DATE(inToDate);
    END IF;
END;